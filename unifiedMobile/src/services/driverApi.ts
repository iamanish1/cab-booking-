...
import type { UnifiedSession, DriverProfile } from "../types";
...
export async function verifyOtp(payload: {
  ...
  const session: UnifiedSession = {
    ...
    profile: data.driver as DriverProfile,
  };
...
export function updateStatus(driverId: string, status: "ONLINE" | "OFFLINE" | "BUSY") {
  return apiRequest(`/drivers/${driverId}/status`, { method: "POST", body: { status } });
}

export function updateLocation(driverId: string, lat: number, lng: number) {
  return apiRequest(`/drivers/${driverId}/location`, { method: "POST", body: { lat, lng } });
}
...
export function verifyRideOtp(rideId: string, otp: string) {
  return apiRequest(`/drivers/rides/${rideId}/verify-otp`, { method: "POST", body: { otp } });
}

export function completeRide(rideId: string, finalFare: number) {
  return apiRequest(`/drivers/rides/${rideId}/complete`, { method: "POST", body: { finalFare } });
}

export function cancelRide(rideId: string, reason: string) {
  return apiRequest(`/drivers/rides/${rideId}/cancel`, { method: "POST", body: { reason } });
}

export function acceptRideOffer(rideId: string, driverId: string) {
  return apiRequest(`/dispatch/${rideId}/accept`, { method: "POST", body: { driverId } }, { auth: false });
}

export function rejectRideOffer(rideId: string, driverId: string) {
  return apiRequest(`/dispatch/${rideId}/reject`, { method: "POST", body: { driverId } }, { auth: false });
}
